/* Fiche de TP1 */
data ex1;   /* on crée le tableau ex1 */
...
data physique1;
  ...
  proc sort data=math1;
   by nom; run;
   proc print data=math1; run;
proc sort data=physique1;
   by nom; run;
   proc print data=physique1; run;
 data note1;
...
data auto2;
  set auto1; 
  ARRAY varCar $ _CHARACTER_ ;
	IF NMISS(OF _NUMERIC_)>0 THEN DELETE ;  /*  si le nombre de variables numeriques qui ont une valeur manquante 
	dépasse >0, alors on efface la ligne */
	DO OVER varCar ;
		IF MISSING(varCar) THEN DELETE ;  /* si dans la suite de caractère il y a une place manquante, 
		alors on efface */
	END ;
RUN ;
   ...
   data question3;
   set auto2; 
     conso=(4.5/1.609)/mpg;  /* litres/km */
	 conso2=conso*100; /* conso litres/100km */
	 run;
...
data euro;
	  ...
	  keep mpg accel weight price;   /* garder seulement ces 4 variables */ 
	   ...
	   put mpg accel weight price; run;    /* on écrit toutes les variables du tableau EURO */
data asie;
	  set question3;
...
proc means data=final;
 ...
 proc plot data=final;
  ...
  proc plot data=final;
    plot note_math*note_physique $note_svt; /* pour avoir toute la valeur de l
	a note en SVT */
	run;
